...
public final class Lab2 {
	...
	public static String interlace(String first, String second, int n) {
		...
		result +=first;
		result +=interlace(second, first, n-1);
	...
	public static String getSubstring(String str, char open, char close) {
		...
		if(open=='\0' && close=='\0') {
	           ...
	           return getSubstring(str.substring(str.indexOf(open)+1),'\0',close);
	       }
		
		else if(str.indexOf(close)!=-1) {
	           
	           return getSubstring(str.substring(0,str.indexOf(close)),open,'\0');
	       }
